...
//(function () {
    ...
    const defineReadOnly = (target, props) => {
        for (const key_value of Object.entries(props)) {
    ...
    var a11y = (shower) => {
        const container = shower.container;
    ...
    var next = (shower) => {
        const stepSelector = shower.options.stepSelector;
        const activeSlideClass = shower.options.activeSlideClass;
        const visitedSlideClass = shower.options.visitedSlideClass;
    ...
    var progress = (shower) => {
        const progressSelector = shower.options.progressSelector;
        ...
        const updateProgress = () => {
            ...
            const length = shower.slides.length;
    ...
    var parseTiming = (timing) => {
        ...
        let sec_min_hour = parsed.map(Number);
        let sec = sec_min_hour[0];
        let min = sec_min_hour[1];
        let hour = sec_min_hour[2];
    ...
    var timer = (shower) => {
        ...
        const resetTimer = () => {
            ...
            const slide = shower.activeSlide;
            const visitCount = slide.state.visitCount;
            const innerStepCount = slide.state.innerStepCount;
    ...
    var view = (shower) => {
        const container = shower.container;
        const fullModeClass = shower.options.fullModeClass;
        const listModeClass = shower.options.listModeClass;
        ...
        const updateScale = () => {
            ...
            const offsetWidth = firstSlide.element.offsetWidth;
            const offsetHeight = firstSlide.element.offsetHeight;
    ...
    var mouse = (shower) => {
        const mouseHiddenClass = shower.options.mouseHiddenClass;
        const mouseInactivityTimeout = shower.options.mouseInactivityTimeout;
    ...
    class Shower extends EventTarget {
        ...
        constructor(options) {
            ...
            const thisOptions = new Object(defaultOptions);
            Object.assign(thisOptions, options);
            defineReadOnly(this, {
                options: thisOptions//{ ...defaultOptions, ...options },
            });
